@extends('layouts.app')
@section('content_title', 'Tambah Delivery Order')
@section('content')
    <form action="{{ route('delivery-order.store') }}" method="POST" id="formDeliveryOrder">
        @csrf
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="date" id="tanggal_do" name="tanggal_do" class="form-control"
                                value="{{ old('tanggal_do', date('Y-m-d')) }}" required>
                            @error('tanggal_do')
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="number" class="form-control" id="total_berat" 
                                   ...
                                   value="{{ old('total_berat') }}" step="0.01" min="0" required>
                            @error('total_berat')
                                <small class="text-danger">{{ $message }}</small>
                            @enderror
                        </div>
        ...
        <div class="card">
            <div class="card-body">
                <button type="submit" class="btn btn-primary">
                    <i class="fas fa-save"></i> Simpan & Lanjut ke Pembelian
                </button>
...
@push('scripts')
    <script>
        $(document).ready(function() {
            ...
            // Jika validasi gagal restore input lama select2
            @if(old('peternak_id'))
                @php $oldPeternakDO = \App\Models\Peternak::find(old('peternak_id')); @endphp
                @if($oldPeternakDO)
                    let oldPeternakOption = new Option(@json($oldPeternakDO->nama), {{ old('peternak_id') }}, true, true);
