...
class MortalitasAyamController extends Controller
{
    public function index()
    {
        ...
        $batch = BatchPembelian::where('stok_ekor', '>', 0)->get();
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            $oldBatch = BatchPembelian::findOrFail($mortalitas->batch_pembelian_id);

            // Revert stok lama
            $oldBatch->stok_ekor += $mortalitas->jumlah_ekor;
            $oldBatch->stok_kg += $mortalitas->berat_kg;
            $oldBatch->save();

            // Ambil batch baru (bisa sama atau beda)
            $newBatch = BatchPembelian::findOrFail($request->batch_pembelian_id);

            if ($newBatch->stok_ekor < $request->jumlah_ekor) {
                throw new Exception('Stok ekor pada batch pembelian tidak mencukupi.');
            }

            // Kurangi stok baru
            $newBatch->stok_ekor -= $request->jumlah_ekor;
            $newBatch->stok_kg -= $request->berat_kg;
            $newBatch->save();
        ...
        toast()->success('Data mortalitas diperbarui');
        return redirect()->route('mortalitas-ayam.index');
    }
    ...
    public function destroy($id)
    {
        DB::transaction(function () use ($id) {
            ...
            // Revert stok
            $batch->stok_ekor += $mortalitas->jumlah_ekor;
            $batch->stok_kg += $mortalitas->berat_kg;
            $batch->save();
